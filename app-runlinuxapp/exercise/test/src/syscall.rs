...
fn sys_mmap(
    ...
    map_flags |= MappingFlags::from(prot);

    let mut aspace = current().task_ext().aspace.lock();
    let vaddr = if addr.is_null() {
        // Use a simple fixed high user-space region for mmap allocations in this exercise.
        VirtAddr::from(0x1000_0000usize)
    } else {
        VirtAddr::from(addr as usize)
    };

    if let Err(_e) = aspace.map_alloc(vaddr, length, map_flags, true) {
    ...
    if !mmap_flags.contains(MmapFlags::MAP_ANONYMOUS) && fd >= 0 {
        ...
        // Preserve original file offset by seeking back after reading mapping content.
        let old_off = api::sys_lseek(fd, 0, 1);
        if old_off < 0 {
            return old_off as _;
        }

        let seek_ret = api::sys_lseek(fd, offset as isize, 0);
        if seek_ret < 0 {
            return seek_ret as _;
        }

        let mut remain = length;
        let mut cursor = vaddr.as_usize();
        let mut buf = vec![0u8; 4096];
        while remain > 0 {
            let chunk = core::cmp::min(remain, buf.len());
            let n = api::sys_read(fd, buf.as_mut_ptr() as _, chunk);
            if n < 0 {
                let _ = api::sys_lseek(fd, old_off as isize, 0);
                return n as _;
            }
            if n == 0 {
                break;
            }
            if let Err(_e) = aspace.write(VirtAddr::from(cursor), &buf[..n as usize]) {
                let _ = api::sys_lseek(fd, old_off as isize, 0);
                return -LinuxError::EFAULT.code() as _;
            }
            cursor += n as usize;
            remain -= n as usize;
        }
