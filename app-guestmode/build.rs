...
fn main() {
    ...
    // `skernel` is now built as a bin target in this package; do not force
    // ArceOS linker scripts unless we are building the main kernel (axstd).
    if std::env::var_os("CARGO_FEATURE_AXSTD").is_none() {
        return;
    }
