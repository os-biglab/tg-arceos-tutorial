...
/// ArceOS ramfs-rename multi-architecture build & run tool
#[derive(Parser)]
#[command(
    name = "xtask",
    about = "Build and run arceos-ramfs-rename on different architectures"
)]
struct Cli {
    #[command(subcommand)]
    command: Cmd,
...
/// Zero-byte raw image for virtio-blk.
/// Axfs still sees a block device; capacity 0 yields no partitions, so it mounts ramfs as root.
fn empty_disk(path: &Path) {
    ...
    println!("Created disk image: {}", path.display());
}
    ...
    match cli.command {
        ...
        Cmd::Run { ref arch } => {
            ...
            let elf = root
                ...
                .join(info.target)
                .join("release")
                .join("arceos-ramfs-rename");
            let bin = elf.with_extension("bin");

            // Empty disk.img: virtio-blk for axfs init, then ramfs root (see `empty_disk`).
